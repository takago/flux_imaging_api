...
# ファイルサーバのベースURL（環境変数から取得、デフォルトは localhost）
FILE_SERVER = os.getenv("FILE_SERVER", "http://localhost:8010")
...
if MODEL_INFO["edit"]["loras"]:
    for i, lora in enumerate(MODEL_INFO["edit"]["loras"]):
        ...
        pipe_edit.set_adapters([adapter_name], adapter_weights=[lora["adapter_weight"]])
...
if MODEL_INFO["generate"]["loras"]:
    for i, lora in enumerate(MODEL_INFO["generate"]["loras"]):
        ...
        pipe_gen.set_adapters([adapter_name], adapter_weights=[lora["adapter_weight"]])
...
def detect_mode(input_image_url, prompt):
    if input_image_url and prompt:
        return "edit"
    elif input_image_url and not prompt:
        return "variation"
    elif not input_image_url and prompt:
        return "generate"
    else:
        return None
...
async def run_pipeline(input_image_url, prompt, bearer_token, seed, width, height,
    ...
    pipeline_mode = detect_mode(input_image_url, prompt)
    if pipeline_mode is None:
        return JSONResponse({"error": "Both input_image_url and prompt are missing"}, status_code=400)

    if pipeline_mode == "variation":
        mode_type = "variation"
        if seed is None:
            seed = DEFAULTS["variation"]["seed"]
        if guidance_scale is None:
            guidance_scale = DEFAULTS["variation"]["guidance_scale"]
        if num_inference_steps is None:
            num_inference_steps = DEFAULTS["variation"]["num_inference_steps"]
        ...
        mode_type = "edit"
        if seed is None:
            seed = DEFAULTS["edit"]["seed"]
        if guidance_scale is None:
            guidance_scale = DEFAULTS["edit"]["guidance_scale"]
        if num_inference_steps is None:
        ...
        if guidance_scale is None:
            guidance_scale = DEFAULTS["generate"]["guidance_scale"]
        if num_inference_steps is None:
    ...
    if mode == "json":
        ...
        return JSONResponse({
            ...
            "model_info": {
                "mode": mode_type,
                "base_model": MODEL_INFO[mode_type]["base_model"],
                "loras": MODEL_INFO[mode_type]["loras"]
            }
        })
    elif mode == "raw":
        buf = io.BytesIO()
...
async def process_image(
    ...
    return await run_pipeline(
        input_image_url, prompt, bearer_token, seed, width, height,
        guidance_scale, num_inference_steps, mode="json"
    )
